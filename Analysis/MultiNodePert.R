                      ...
                      randChoice = F, nState = 100, nIter = 10, sMax = 25, write=T)
{
    ...
    if (write)
        write_csv(df, paste0(WTcoherence, "/", net, "_", nNode, "_coherence.csv"), 
                  quote = "none")
    else
        return(df)
}
...
coherenceSingleNode <- function(net, nCores = 8)
{
...
coherenceSingleNodeAlt <- function(net)
{
    df <- coherence(net, randChoice = F, nIter = 50, write = F) 
    dfss <- read_csv(paste0(net, "_finFlagFreq.csv"), show_col_types = F, lazy=F)
    if (all(is.na(df)))
    {
        dfss$coherence0 <- NA
        return(dfss)
    ...
    if (any(str_detect(colnames(dfss), "coherence")))
        dfss <- dfss %>% select(-contains("coherence"))
    df <- merge(df, dfss, by = "states", all = T)
}
coherenceSingleNodeAlt <- cmpfun(coherenceSingleNodeAlt)
